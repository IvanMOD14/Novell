...
style namebox:
    ...
    background Frame(gui.namebox_background, gui.namebox_borders, tile=gui.namebox_tile, xalign=gui.name_xalign)
    ...
    if quick_menu2:
        frame:
            ...
            vbox:
                ...
                imagebutton:
                    idle  "gui/hide/toggle_button_normal.png"
                    hover "gui/hidetoggle_button_hover.png"
                    clicked "gui/hidetoggle_button_down.png"
                    action QuickSave()
    ...
    if quick_menu:
        ...
        frame:
            ...
            hbox:
                ...
                textbutton _("숨기기") action HideInterface()  
...
screen navigation():
    ...
    vbox:
        ...
        if renpy.get_screen("main_menu"):
            textbutton _("갤러리") action ShowMenu("gallery_navigation")
    ...
    if renpy.get_screen("main_menu"):
        imagebutton:
            idle Transform("gui/logo/patreon2.png", zoom=0.15)  # 일반 상태 이미지
            hover Transform("gui/logo/patreon2_hover.png", zoom=0.15)   # 마우스 오버 상태 이미지
            action [Play("sound", "click.mp3"), OpenURL("https://www.patreon.com/Arcane_shop")]
            xcenter 0.95
            ycenter 0.9
    ...
    if renpy.get_screen("main_menu"):
        imagebutton:
            idle Transform("gui/logo/twitter.png", zoom=0.15)  # 일반 상태 이미지
            hover Transform("gui/logo/twitter_hover.png", zoom=0.15)   # 마우스 오버 상태 이미지
            action [Play("sound", "click.mp3"), OpenURL("https://twitter.com/Ronci14")]
            xcenter 0.88
            ycenter 0.9

    if renpy.get_screen("main_menu"):
        imagebutton:
            idle Transform("gui/logo/itch.png", zoom=0.15)  # 일반 상태 이미지
            hover Transform("gui/logo/itch_hover.png", zoom=0.15)   # 마우스 오버 상태 이미지
            action [Play("sound", "click.mp3"), OpenURL("https://ronci14.itch.io/arcane-shop")]
            xcenter 0.95
            ycenter 0.775
...
screen language_selector():
    ...
    frame:
        ...
        ysize 690  # 박스의 높이 설정
        background Solid("#fdfdfdf8")  # 박스의 배경으로 Solid 적용
        
        vbox:
            xalign 0.5
            spacing 30
            
            label "언어를 선택해 주세요. Choose a language.":
                xalign 0.5
            ...
            hbox:
                ...
                vbox:
                    ...
                    textbutton "한국어":
                        action [
                            Language(None), 
                            SetVariable("persistent.language_selected", True),
                            Hide("language_selector", transition=dissolve)
                        ]
                        text_style "language_selector"
                    
                    textbutton "English":
                        action [
                            Language("english"), 
                            SetVariable("persistent.language_selected", True),
                            SetVariable("default_name", "Resque"),
                            SetVariable("pl_name", "Resque"),
                            Hide("language_selector", transition=dissolve)
                        ]
                        text_style "language_selector"
...
screen main_menu():
    ...
    # main_menu 스크린이 표시될 때 자동으로 언어 선택 확인
    on "show":
        action [
            If(persistent.language_selected == False, Show("language_selector")),
            SetVariable("pl_name", "레스크" if _preferences.language == None else "Resque")
        ]
            ...
            hbox:
                ...
                if renpy.variant("pc") or renpy.variant("web"):
                    ...
                    vbox:
                        ...
                        label _("데이터")
                        textbutton _("데이터 초기화") action Show("confirm_clear_data")
                ...
                if renpy.variant("pc") or renpy.variant("web"):
                    ...
                    vbox:
                        style_prefix"radio"
                        label _("Language")
                        textbutton _ ("한국어")action Language (None)
                    ...
                    vbox:
                        style_prefix"radio"
                        label ""
                        textbutton _("簡体中文")action Language("chinese_simple")
                        textbutton _("繁體中文") action Language("chinese_traditional")
                        textbutton _("Tiếng Việt") action Language("vietnamese")
...
init python:
    ...
    def delete_all_saves():
        # 모든 세이브 삭제 (일반 세이브, 자동 저장, 퀵세이브 포함)
...
screen confirm_clear_data():
    ...
    frame:
        vbox:
            ...
            label _("정말로 모든 데이터를 삭제하고 게임을 초기화하시겠습니까?\n{color=#ee3939}이 작업은 다시 되돌릴 수 없습니다.{/color}"):
                style "confirm_prompt"
                xalign 0.5
            ...
            hbox:
                ...
                textbutton _("네") action Show("confirm_clear_data_final")
                textbutton _("아니오") action Hide("confirm_clear_data")

    # 우클릭과 esc는 '아니오'를 입력하는 것과 같습니다.
    key "game_menu" action Hide("confirm_clear_data")

screen confirm_clear_data_final():
    modal True
    zorder 201  # 첫 번째 확인 스크린보다 위에 표시
    style_prefix "confirm"
    ...
    frame:
        vbox:
            ...
            label _("{color=#ee3939}정말로 초기화하시겠습니까?{/color}"):
                style "confirm_prompt"
                xalign 0.5
            ...
            hbox:
                ...
                textbutton _("아니오, 취소합니다") action [Hide("confirm_clear_data"), Hide("confirm_clear_data_final")]
                textbutton _("네, 초기화합니다") action [Function(clear_persistent_data), Hide("confirm_clear_data"), Hide("confirm_clear_data_final")]
                

    key "game_menu" action [Hide("confirm_clear_data"), Hide("confirm_clear_data_final")]
...
screen affection_indicator():
    ...
    style_prefix "affection"
    timer 3.25 action Hide("affection_indicator", dissolve)
    
    if renpy.variant("mobile"):
        frame:
            style "mobile_affection_frame"
            at notify_appear
...
screen affection_indicator2():

    zorder 100
    
    style_prefix "affection"
    timer 3.25 action Hide("affection_indicator2", dissolve)
...
define gui.affection_frame_borders = Borders(25, 8, 95, 8)
...
style affection_frame is empty
style affection_text is gui_text
...
style affection_frame:
    ...
    yalign -0.1
    ypos 68
    background Frame("gui/white_box.png", gui.affection_frame_borders, tile=gui.frame_tile)
    padding gui.affection_frame_borders.padding
...
style affection_text:
    size gui.notify_text_size


style affection2_frame is empty
style affection2_text is gui_text
...
screen patreon_button():
    ...
    imagebutton:
        idle Transform("gui/logo/patreon.png")  # 일반 상태 이미지
        hover Transform("gui/logo/patreon_hover.png")   # 마우스 오버 상태 이미지
        action [Play("sound", "click.mp3"), OpenURL("https://www.patreon.com/Arcane_shop")]
        xcenter 0.85
        ycenter 0.235
...
init python:
    # 파일 읽기 (바이너리 모드)
    file_contents = renpy.file('script/supporters.txt').read()

    # 바이너리 데이터를 UTF-8 텍스트로 디코드
    file_contents = file_contents.decode("utf-8")

    # 파일 내용을 줄 단위로 분리하여 리스트 만들기
    supporters = file_contents.splitlines()

    # 선택적: 비어 있는 줄 제거
    supporters = [line for line in supporters if line.strip()]
...
screen credits():
    ...
    frame:
        ...
        # 박스 내부에 텍스트를 중앙에 배치하기 위한 vbox
        vbox:
            align (0.5, 0.5)  # 내부 vbox를 중앙에 정렬
            text "Special Thanks" size 50 color "#000000" font "NanumSquareEB.ttf"
            spacing 50  # 텍스트 간격
            ...
            grid 1 7:
                ...
                hbox:
                    textbutton "'노말회원'":
                        action OpenURL("https://x.com/NML_inChaos")
                        text_style "credits_link_kr"
                    text " write the story." size 30 color "#000000" font "NanumSquareR.ttf"

                hbox:
                    textbutton "'천둥소'" :
                        action OpenURL("https://x.com/Thunderbull_1")
                        text_style "credits_link_kr"
                    text " for reviewing the English translation." size 30 color "#000000" font "NanumSquareR.ttf"

                hbox:
                    textbutton "'ryandrake_Mh'":
                        action OpenURL("https://x.com/RyanDrake_MH")
                        text_style "credits_link"
                    text " provided the Spanish translation." size 30 color "#000000" font "NanumSquareR.ttf"

                hbox:
                    textbutton "'mikagetantian'":
                        action OpenURL("https://x.com/MikageTantian")
                        text_style "credits_link"
                    text ", " size 30 color "#000000" font "NanumSquareR.ttf"
                    textbutton "'Dragonify'":
                        action OpenURL("https://x.com/Drag0nify")
                        text_style "credits_link"
                    textbutton "'［moriki］zuye '":
                        action OpenURL("https://x.com/john870911")
                        text_style "credits_link"
                    text " provided the Chinese translation." size 30 color "#000000" font "NanumSquareR.ttf"

                hbox:
                    textbutton "'pixiu'":
                        text_style "credits_link"
                    text " reviewing the Chinese translation" size 30 color "#000000" font "NanumSquareR.ttf"

                hbox:
                    textbutton "'ExGames'" :
                        action OpenURL("https://discord.com/users/897127067626405898")
                        text_style "credits_link"
                    text " provided the Russian translation." size 30 color "#000000" font "NanumSquareR.ttf"

                hbox:
                    textbutton "'Scubidu'" :
                        action OpenURL("https://x.com/drag_oni")
                        text_style "credits_link"
                    text ", " size 30 color "#000000" font "NanumSquareR.ttf"
                    textbutton "'thaile'" :
                        text_style "credits_link"
                    text " provided the Vietnamese translation." size 30 color "#000000" font "NanumSquareR.ttf"
...
style credits_link:
    ...
    color "#000000"
    hover_color "#FF0000"
    font "Source Han Sans CN Medium.otf"
    yoffset -5

style credits_link_kr:
    size 30
    color "#000000"
    hover_color "#FF0000"
...
screen special_thanks():
    ...
    frame:
        ...
        vbox:
            ...
            # 후원자 이름을 3열 6행 그리드로 표시
            grid 5 7:
                xspacing 40
                yspacing 10
                # 후원자의 이름을 각 셀에 표시
                for supporter in supporters:
                    text supporter size 30 color "#000000" font "SourceHanSansLite.ttf"
...
screen quick_menu():
    ...
    if quick_menu:
        frame:
            ...
            background "gui/phone/quickbox.png"
            xalign 0.93
            yalign 0.995
...
style namebox:
    variant "small"
    xpos 0.03  # 예: 왼쪽으로부터 10% 떨어진 위치
    ypos -0.07  # 예: 상단으로부터 10% 떨어진 위치

style say_dialogue:
    variant "small"
    size 41  # 예: 폰트 크기를 18pt로 설정

style quick_button_text:
    variant "small"
    size 27  # 모바일 환경에서의 폰트 크기 조정
