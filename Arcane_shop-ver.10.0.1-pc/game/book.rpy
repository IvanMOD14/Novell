...
init python:
    def unlock_character_image(char_id, image_id):
        ...
        # Args:
        #     char_id (str): 캐릭터 ID (예: "cara", "wolf")
        #     image_id (str): 이미지 ID (예: "cara b_no inn_d")
...
init python:
    def unlock_multiple_images(char_id, image_list):
        """
        여러 이미지를 한번에 언락합니다.
        
        Args:
            char_id (str): 캐릭터 ID
            image_list (list): 언락할 이미지 ID 리스트
        """
        for image_id in image_list:
            unlock_character_image(char_id, image_id)
        renpy.save_persistent()
